...
Use these sections for the default persistent report.
Honor the explicit format and write restrictions in SKILL.md; do not fabricate unavailable evidence to fill a section.
...
**Line number references:** Include when available; otherwise record the evidence limitation
...
Use the user-requested authorized path, otherwise a suitable project report path.
Do not overwrite unrelated existing reports; choose a unique filename within that scope.
For explicit chat-only or read-only requests, deliver the review in chat and write no report file.
Do not use Desktop, global skill directories, or a different repository as fallback destinations.
...
- Adapt the local report for stakeholders when requested
````

---

## Further formatting and write failures

Use the report above directly or adapt its format when the user requests a stakeholder version.
External issue creation and PR posting are separate authorized operations, not an automatic review phase.
If writing the authorized report fails, state the exact limitation and deliver findings in chat.
Do not expand filesystem permissions or switch destinations merely to complete auxiliary persistence.
Persistent output remains the default when authorized and no different format was requested.
